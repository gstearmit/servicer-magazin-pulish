...
class UploadlibrarybooksTable extends AbstractTableGateway {
    ...
    public function __construct(Adapter $adapter) {
        ...
        $this->resultSetPrototype->setArrayObjectPrototype(new Uploadlibrarybooks());
...
//     	$sort[] = 'value ASC';
 //    	$select->order($sort);
    ...
    public function getReadUploadlibrarybooksdetail($id)
    {
    	$id = (int) $id;
    ...
    public function saveUploadlibrarybooks(Uploadlibrarybooks $upload) {
        $data = array(
    ...
    public function getInsertUploadDetail($arrayImages = Array() , $id , $namefolder)
    {
    	...
    	if (is_array($arrayImages) and !empty($arrayImages))
    	{
    		...
    		foreach ($arrayImages as $key => $imgvalue)
    		{
    			...
    			$sql       = "INSERT INTO librarydetail (idmz,img,description,title,page)
                              VALUES ('".$id."','".$img."','','','".$i."')";
