...
namespace Uploadmagazinevietnam\Controller;

use Zend\Mvc\Controller\AbstractActionController;
use Zend\View\Model\ViewModel;
use Uploadmagazinevietnam\Model\Uploadmagazinevietnam as UploadmagazinevietnamModel;
use Uploadmagazinevietnam\Model\UploadmagazinevietnamTable;
use Uploadmagazinevietnam\Form\UploadmagazinevietnamForm;
...
class UploadmagazinevietnamController extends AbstractActionController {
	...
	public function uploadnewAction() 
	{
		...
		$form = new UploadmagazinevietnamForm ($dbAdapter);
		//Upload
		...
		if ($request->isPost ())
		{
			...
			$upload = new UploadmagazinevietnamModel();
			...
			if ($form->isValid()) {
				  ...
				  // Save Database 
					...
					$id_curent_row_upload = $this->getUploadmagazinevietnamTable()->saveUploadmagazinevietnam($upload);
							...
							if (move_uploaded_file ( $source, $myFile ))
							 {
							    ...
							    if($returnResult === 1)
							    {
							    	$backUrl = $this->url()->fromRoute('uploadmagazinevietnam', array('action' => 'readdetail', 'id' => $id_curent_row_upload), array(), true);
							    }else {
	...
	public function readdetailAction()
	{
		...
		if (! $id) {
			return $this->redirect ()->toRoute ( 'uploadmagazinevietnam' );
		}
	
		$read = $this->getUploadmagazinevietnamTable ()->getReadUploadmagazinevietnamdetail( $id ) ;
		return array (
				'id' => $id,
				'readdetail' => $read,
		);
	...
	public function getUploadmagazinevietnamTable() {
		if (! $this->magazinepublish) {
			$sm = $this->getServiceLocator ();
			$this->magazinepublish = $sm->get ( 'Uploadmagazinevietnam\Model\UploadmagazinevietnamTable' );
		}
