...
use Upload\Model\Upload as UploadModel;
...
// 				echo '</pre>';
				  ...
				  // Save Database 
					...
					$id_curent_row_upload = $this->getUploadTable()->saveUpload($upload);
...
// 								echo '</pre>';
								...
								$returnResult = $this->getUploadTable()->getInsertUploadDetail($imgArray , $id_curent_row_upload, $name[0]);
								
							    if($returnResult === 1)
							    {
							    	$backUrl = $this->url()->fromRoute('upload', array('action' => 'readdetail', 'id' => $id_curent_row_upload), array(), true);
							    }else {
	...
	public function readdetailAction()
	{
		...
		$read = $this->getUploadTable ()->getReadUploaddetail( $id ) ;
		return array (
				'id' => $id,
				'readdetail' => $read,
		);
